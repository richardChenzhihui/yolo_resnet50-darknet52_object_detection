...
class mmdetection_head(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, do_downsample=False):
        ...
        #self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=2, bias=False, dilation=2)  # dilation
        ...
        if do_downsample or stride != 1 or in_planes != self.expansion * planes:
            self.downsample = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion * planes, stride = stride, kernel_size=1, bias=False)
            )
        ##################################################################
