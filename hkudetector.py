...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, args):
        ...
        ##################################################################
        self.conv1_x = self._make_layer(self.block, 64, layers[0], stride=1)
        self.conv2_x = self._make_layer(self.block, 128, layers[1], stride=2)
        self.conv3_x = self._make_layer(self.block, 256, layers[2], stride=2)
        self.conv4_x = self._make_layer(self.block, 512, layers[3], stride=2)
        ##################################################################
        ...
        out_c = 5 * yolo_B + yolo_C
        self.det_head = self._make_detection_head(in_channels=2048, out_channels=out_c)
        ##################################################################
