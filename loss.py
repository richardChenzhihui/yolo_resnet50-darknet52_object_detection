...
class yololoss(nn.Module):
    def __init__(self, args, l_coord, l_noobj):
        # parser.add_argument('--l_coord,help='hyper parameter for localization loss')
    ...
    def forward(self, prediction, target):
        ...
        loss_l = [loc_loss, contain_loss, not_response_loss, nooobj_loss, class_loss]
        #loss = self.l_coord * loc_loss + contain_loss + self.l_noobj * (not_response_loss + nooobj_loss) + 3*class_loss
        loss = self.l_coord * loc_loss + contain_loss + self.l_noobj * (not_response_loss + nooobj_loss) + class_loss
        return loss / N, loss_l
